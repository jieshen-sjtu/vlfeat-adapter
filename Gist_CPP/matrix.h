...
void squaresqrt(matrix<int> x,matrix<int> y){
		assert(x.row == row);
		assert(x.col == col);
		assert(y.row == row);
		assert(y.col == col);
...
void fftshift(){
		...
		if (col%2 == 0){
			...
			//printf("\nffffffuuuuuuuuuuuuucccccccccckkkkkkkkkkk\n");
		}
}

void angle(matrix<int> x,matrix<int> y){
		double a,b;
		double alfa;
...
void add(matrix<double> ori,double d){
		assert(row = ori.row);
		assert(col = ori.col);
		for (int i=0;i<row;i++)
			for(int j=0;j<col;j++)
...
void squareexps1(matrix<int> fx,matrix<int> fy,double s1){
		double ps = pow(s1,2);
		double dtmp;
		for (int i=0;i<row;i++)
			for (int j=0;j<col;j++){
